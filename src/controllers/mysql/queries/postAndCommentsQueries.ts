...
export function selectAllContentByUsername(username:string){
    ...
    connection.query('SELECT user.id as userId, user.username AS userName, comment.`content` AS commentContent, comment.`time` as commentTime, comment.`id` as commentId, comment.`fk_comment` as commentNumber, comment.`fk_post` as postId, post.title as post_title, post.`url` as post_url, post.`time` as postTime '
    + 'FROM user '+
...
export function selectAllContentByUserId(userid:number){
  ...
  export function selectAllContentByPostId(postid:number){
    return new Promise((resolve) => {
    connection.query('SELECT user.id as userId, user.username AS userName, comment.`content` AS commentContent, comment.`time` as commentTime, comment.`id` as commentId, comment.`fk_comment` as commentNumber, comment.`fk_post` as postId, post.title as postTitle, post.`url` as post_url, post.`time` as postTime '
    + 'FROM user '+
    'JOIN comment ON user.id = comment.`fk_user` '+
    'join post ON post.`id` = comment.`fk_post` '+
    'where post.id =?'
  ...
  export function selectAllContentByPostTitle(postTitle:string){
    return new Promise((resolve) => {
    connection.query('SELECT user.id as userId, user.userName AS user, comment.`content` AS commentContent, comment.`time` as commentTime, comment.`id` as commentId, comment.`fk_comment` as commentNumber, comment.`fk_post` as postID, post.title as postTitle, post.`url` as post_url, post.`time` as postTime '
    + 'FROM user '+
    'JOIN comment ON user.id = comment.`fk_user` '+
    'join post ON post.`id` = comment.`fk_post` '+
    'where post.title =?'
